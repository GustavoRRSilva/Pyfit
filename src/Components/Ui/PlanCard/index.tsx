...
export default function PlanCard({betterBenefit}:CardPlan) {
   ...
   return(
    <div className={styles.card}>
       {betterBenefit && <span className={styles.betterBenefit}>Melhor custo Beneficio</span>}
        <h2 className={styles.planType}>Plano {betterBenefit? "Plus" :"Basic"}</h2>
        <p className={styles.whiteComum}>A partir de </p>
        <h3 className={styles.valuePlan}>R$<b>{betterBenefit? "0,99" :"99,90"}</b></h3>
        <p className= {styles.whiteComum}>{betterBenefit ? "no primeiro mês" : "por mês"}</p>
        <button className = {styles.toEnroll}>MATRICULE-SE JÁ!</button>
        <ul className= {styles.benefits}>
        {benefits.map((benefit,index)=>(
         <li key={benefit} className={styles.benefit}>
            <Image width={10} height={10} alt="Correct image" src={CorrectImage}/>
            <p>{benefit}</p>
         </li>
      ))}
        </ul>
